...
- [第三步：运行](#第三步运行)
- [新增数据直接注入（new sheet 功能）](#新增数据直接注入new-sheet-功能)
- [输出文件说明](#输出文件说明)
- [输出格式详解](#输出格式详解)
- [切换平台格式](#切换平台格式)
- [语义冲突检测](#语义冲突检测)
...
split:
  ...
  test: 0.1     # 10% 用于测试
  random_seed: 42
  # new_data_sheet：指定"新增数据"所在的 sheet 名称
  #   该 sheet 的数据不参与 8:1:1 划分，而是全量直接追加到 train 和 val
  #   适用于存量大（如 6 万条）、新增少（如 100 条）的场景，确保新数据都被训练到
  #   留空（""）则禁用此功能，所有 sheet 合并后统一划分
  new_data_sheet: "new"
...
## 新增数据直接注入（new sheet 功能）

### 适用场景

当存量数据量很大（如 6 万条），而新增数据很少（如 100 条）时，如果将新数据混入整体按 8:1:1 划分，100 条新数据只有约 80 条进训练集，极易被存量数据淹没，导致模型对新语料学习效果很差。

**解决方案：** 在 Excel 中新建一个独立的 sheet（默认命名 `new`），将新增数据放入该 sheet。程序会自动将这部分数据**全量**追加到 train 和 val，不参与划分，不进 test。

### Excel 结构示例

```
工作簿（.xlsx）
├── Sheet1（或任意名称）  ← 存量数据，走正常 8:1:1 划分
└── new                  ← 新增数据，全量注入 train + val
```

### 配置

在 `config.yaml` 中指定 sheet 名称：

```yaml
split:
  new_data_sheet: "new"   # 改为你的 sheet 名；留空则禁用此功能
```

### 效果

| 数据来源 | 进 train | 进 val | 进 test |
|---------|---------|--------|---------|
| 存量数据（其他 sheet）| 80% | 10% | 10% |
| 新增数据（new sheet）| ✅ 全量 | ✅ 全量 | ❌ 不进 |

> 如果 Excel 中不存在配置的 sheet 名称，此功能自动静默跳过，不影响正常流程。
